...
@section('content')
                                                ...
                                                @if ($item->is_deleted == 0)
                                                    <tr>
                                                        ...
                                                        <td class="crancy-table__column-2 fw-semibold">
                                                            {{ $item->users->fullname ?? '-' }}</td>
                                                        <td class="crancy-table__column-2 fw-semibold">
                                                            {{ $item->transaksi_pengeluaran->nomor_order ?? '-' }}</td>
                                                        <td class="crancy-table__column-2 fw-semibold">
                                                            {{ $item->bonus->nama ?? '-' }}</td>
                                                        @if (empty(array_intersect(['staff', 'akuntan'], $userRole)))
                                                            ...
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body text-center p-4">
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <form
                                                                            action="{{ route('manajemen-assign-bonus.destroy', $item->id) }}"
                                                                            method="POST">
                                                                            @csrf
                                                                            @method('DELETE')
                                                                            <button type="submit"
    ...
    <div id="addAssignBonusModal" class="modal fade" tabindex="-1" aria-labelledby="addAssignBonusModalLabel"
        aria-hidden="true">
            ...
            <form method="POST" action="{{ route('manajemen-assign-bonus.store') }}" class="modal-content">
                ...
                <div class="modal-body px-4 pb-5 ">
                    <div class="mb-5 pb-3 form-group ">
                        ...
                        <select id="user_id" class="form-select crancy__item-input fw-semibold" name="user_id"
                            required>
                    ...
                    <div class="mb-5 py-3 form-group">
                        <label class="form-label" for="transaksi_pengeluaran_id">Transaksi <span
                                class="text-danger">*</span></label>
                        <select id="transaksi_pengeluaran_id" class="form-select crancy__item-input fw-semibold"
                            name="transaksi_pengeluaran_id" required>
                    ...
                    <div class="mb-5 py-3 form-group">
                        ...
                        <select id="bonus_id" class="form-select crancy__item-input fw-semibold" name="bonus_id"
                            required>
    ...
    @foreach ($assignBonuses as $item)
        <div id="editAssignBonusModal-{{ $item->id }}" class="modal fade" tabindex="-1"
            aria-labelledby="editAssignBonusModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-scrollable mb-5">
                <form method="POST" action="{{ route('manajemen-assign-bonus.update', $item->id) }}"
                    class="modal-content">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                    ...
                    <div class="modal-body p-4">
                        <div class="mb-5 form-group">
                            ...
                            <select id="editUserId" class="form-select crancy__item-input fw-semibold" name="user_id"
                                required>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ $item->user_id == $user->id ? 'selected' : '' }}>{{ $user->fullname }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="mb-5 form-group">
                            <label class="form-label" for="editTransaksiId">Transaksi <span
                                    class="text-danger">*</span></label>
                            <select id="editTransaksiId" class="form-select crancy__item-input fw-semibold"
                                name="transaksi_pengeluaran_id" required>
                                @foreach ($transaksis as $transaksi)
                                    <option value="{{ $transaksi->id }}"
                                        {{ $item->transaksi_pengeluaran_id == $transaksi->id ? 'selected' : '' }}>
                                        {{ $transaksi->nomor_order }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3 form-group">
                            ...
                            <select id="editBonusId" class="form-select crancy__item-input fw-semibold" name="bonus_id"
                                required>
                                @foreach ($bonuses as $bonus)
                                    <option value="{{ $bonus->id }}"
                                        {{ $item->bonus_id == $bonus->id ? 'selected' : '' }}>{{ $bonus->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
