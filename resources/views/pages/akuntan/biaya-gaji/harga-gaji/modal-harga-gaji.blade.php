<!-- Create Modal -->
<div id="addHargaGajiModal" class="modal fade" tabindex="-1" aria-labelledby="addHargaGajiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable">
        <form method="POST" action="{{ route('harga-gaji.store', $user->id) }}" class="modal-content">
            @csrf
            <div class="modal-header">
                <h5 class="modal-title fs-6" id="addHargaGajiModalLabel">
                    Tambah Harga Gaji Baru untuk "{{ $user->username ? $user->fullname : 'N/A' }}"
                </h5>
...
<!-- Edit Modal -->
@foreach ($harga_gaji as $data)
    <div id="editHargaGajiModal-{{ $data->id }}" class="modal fade" tabindex="-1"
        aria-labelledby="editHargaGajiModalLabel-{{ $data->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <form method="POST" action="{{ route('harga-gaji.update', $data->id) }}" class="modal-content">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title fs-6" id="editHargaGajiModalLabel-{{ $data->id }}">
                        Edit Harga Gaji "{{ $user->username ? $user->fullname : 'N/A' }}"
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <!-- Delete Modal -->
    <div id="removeHargaGajiModal-{{ $data->id }}" class="modal fade zoomIn" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-center p-4">
                    ...
                    <p class="text-muted mb-4">
                        Apakah kamu yakin ingin menghapus harga gaji ini?
                        <strong>Data yang dihapus tidak dapat diaktifkan kembali.</strong>
                    </p>
                    <div class="d-grid gap-2 d-md-flex justify-content-md-center">
                        <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Batal</button>
                        <form action="{{ route('harga-gaji.destroy', $data->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
