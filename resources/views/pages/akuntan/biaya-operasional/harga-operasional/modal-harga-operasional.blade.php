<!-- Create Modal -->
<div id="addHargaOperasionalModal" class="modal fade" tabindex="-1" aria-labelledby="addHargaOperasionalModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable">
        <form method="POST" action="{{ route('harga-operasional.store', $biaya->id) }}" class="modal-content">
            @csrf
            <div class="modal-header">
                <h5 class="modal-title fs-6" id="addHargaOperasionalModalLabel">
                    Tambah Harga Operasional Baru untuk "{{ $biaya->nama_biaya_operasional }}"
                </h5>
...
<!-- Edit Modal -->
@foreach ($harga_operasional as $data)
    <div id="editHargaOperasionalModal-{{ $data->id }}" class="modal fade" tabindex="-1"
        aria-labelledby="editHargaOperasionalModalLabel-{{ $data->id }}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable">
            <form method="POST" action="{{ route('harga-operasional.update', $data->id) }}" class="modal-content">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title fs-6" id="editHargaOperasionalModalLabel-{{ $data->id }}">
                        Edit Harga Operasional "{{ $biaya->nama_biaya_operasional }}"
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <!-- Delete Modal -->
    <div id="removeHargaOperasionalModal-{{ $data->id }}" class="modal fade zoomIn" tabindex="-1"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-center p-4">
                    ...
                    <p class="text-muted mb-4">
                        Apakah kamu yakin ingin menghapus harga operasional ini?
                        <strong>Data yang dihapus tidak dapat diaktifkan kembali.</strong>
                    </p>
                    <div class="d-grid gap-2 d-md-flex justify-content-md-center">
                        <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Batal</button>
                        <form action="{{ route('harga-operasional.destroy', $data->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
