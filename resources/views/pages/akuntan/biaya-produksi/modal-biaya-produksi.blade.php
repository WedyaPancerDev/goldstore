 ...
 <!-- Modal Edit Biaya Produksi -->
 @foreach ($biaya_produksi as $data)
     <div id="editBiayaProduksiModal-{{ $data->id }}" class="modal fade" tabindex="-1"
         aria-labelledby="editBiayaProduksiModalLabel" aria-hidden="true">
         <div class="modal-dialog modal-dialog-scrollable">
             <form method="POST" action="{{ route('biaya-produksi.update', $data->id) }}" class="modal-content">
                 @csrf
                 @method('PUT')
                 <div class="modal-header">
                     <h5 class="modal-title fs-6" id="editBiayaProduksiModalLabel">Edit Biaya Produksi</h5>
                     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>

                 <div class="modal-body p-4">
                     <div class="mb-3 form-group">
                         <label class="form-label" for="editNama">Nama Biaya Produksi <span
                                 class="text-danger">*</span></label>
                         <input id="editNamaBiayaProduksi" class="crancy-wc__form-input fw-semibold" type="text"
                             name="nama_biaya_produksi" value="{{ $data->nama_biaya_produksi }}" required />
                         @if ($errors->has('nama_biaya_produksi'))
                             <div class="pt-2">
                                 <span
                                     class="form-text fw-semibold text-danger">{{ $errors->first('nama_biaya_produksi') }}</span>
